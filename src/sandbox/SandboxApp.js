...
class SandboxApp extends Component {
  render() {
    const serviceDetailsPath = `/servicedetails/org/${process.env.REACT_APP_SANDBOX_ORG_ID}/service/${process.env.REACT_APP_SANDBOX_SERVICE_ID}`;
    return (
        ...
        <div>
          <Router>
            <Suspense fallback={<div>Loading...</div>}>
              <Switch>
                <Route
                  path={`/servicedetails/org/:orgId/service/:serviceId`}
                  {...this.props}
